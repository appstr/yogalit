class TeacherSundayTimeFramesController < ApplicationController
  ...
  def end_less_than_start?(teacher)
    if Time.zone.local(2017, 04, 02, @start_hour, @start_minute, 00).in_time_zone(teacher[:timezone]) > Time.zone.local(2017, 04, 02, @end_hour, @end_minute, 00).in_time_zone(teacher[:timezone])
      return true
    else
      return false
    end
  ...
  def time_frame_taken?(teacher)
    time_frames = TeacherSundayTimeFrame.where(teacher_id: teacher)
    return false if time_frames.blank?
    time_frames.each do |tf|
      if (tf[:time_range].first..tf[:time_range].last).include? Time.zone.local(2017, 04, 02, @start_hour, @start_minute, 00).in_time_zone(teacher[:timezone]).to_i
        return true
      elsif (Time.zone.local(2017, 04, 02, @start_hour, @start_minute, 00).in_time_zone(teacher[:timezone]).to_i..Time.zone.local(2017, 04, 02, @end_hour, @end_minute, 00).in_time_zone(teacher[:timezone]).to_i).include? tf[:time_range].first
        return true
      end
    end
    return false
  ...
  def teacher_sunday_time_frame_params
    params.require(:teacher_sunday_time_frame).permit(:teacher_id, :time_range)
  end
end
